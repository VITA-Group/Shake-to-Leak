...
def parse_args(input_args=None):
    ...
    parser.add_argument("--train_vae", action="store_true", help="Whether to train the image vae")
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        help="Bias type for Lora. Can be 'none', 'all' or 'lora_only', only used if use_lora and `train_vae` are True",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Lora rank for text encoder, only used if `use_lora` and `train_vae` are True",
    )
    parser.add_argument(
        "--lora_vae_alpha",
        type=int,
        default=32,
        help="Lora alpha for text encoder, only used if `use_lora` and `train_vae` are True",
    )
    parser.add_argument(
        "--lora_vae_dropout",
        type=float,
        default=0.0,
        help="Lora dropout for text encoder, only used if `use_lora` and `train_vae` are True",
    )
    parser.add_argument(
        "--lora_vae_bias",
        type=str,
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    # Concat class and instance experiments for prior preservation.
...
def main(args):
    ...
    elif args.train_vae and args.use_lora:
        config = LoraConfig(
            r=args.lora_vae_r,
    ...
    if args.gradient_checkpointing:
        ...
        if args.train_vae and not args.use_lora:
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(unet):
                    ...
                    if accelerator.sync_gradients:
                        if args.train_text_encoder and args.train_vae:
                            params_to_clip = itertools.chain(unet.parameters(), text_encoder.parameters(), vae.parameters())
                        elif args.train_text_encoder:
                ...
                if (
                    ...
                    pipeline.vae = accelerator.unwrap_model(vae, keep_fp32_wrapper=True)
    ...
    if accelerator.is_main_process:
        if args.use_lora:
            ...
            if args.train_vae:
                unwarpped_vae = accelerator.unwrap_model(vae)
                unwarpped_vae.save_pretrained(
            ...
            pipeline = DiffusionPipeline.from_pretrained(
                ...
                vae=accelerator.unwrap_model(vae), #TODO: bug?
                revision=args.revision,
            )
